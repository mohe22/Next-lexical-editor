...
import { MoreHorizontal, PaintRoller } from 'lucide-react';
...
function TableActionMenu({
  ...
  return createPortal(
    <div
      ...
      z-[100]  absolute min-h-[40px]  flex flex-col space-y-2
      "
      ref={dropDownRef}
      onClick={(e) => {
        e.stopPropagation();
      ...
      <Button
           ...
           size={"none"}
           type="button"
           className="h-[32px] gap-x-3 px-2"
            onClick={() =>
              {
                onClose()
                setOpen(
                  <CustomModal title='Color'  className='items-center w-fit justify-center' subheading='background color'>
                    <ColorPicker
        ...
        <PaintRoller style={{color:backgroundColor}} className={"w-4 h-4"}/>
      ...
      {tableCellNode != null && (
        <>
          <Button
            ...
            variant={"ghost"}
            className={
              `bg-none flex  p-1 opacity-60 transition-all hover:opacity-100 items-center justify-center border-0 rounded-md cursor-pointer`
            }
            onClick={(e) => {
              e.stopPropagation();
